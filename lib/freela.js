/*!
 * @package: calculadora-freelancer
 * @description: Calculadora freelancer - Calcula valor hora e o valor do projeto
 * @version: 1.0.0
 * @author: Fernando Moreira <nandomoreira.me@gmail.com> (https://nandomoreira.me)
 * @license: Licensed under the MIT
 */
(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		module.exports = factory();
	else if(typeof define === 'function' && define.amd)
		define("Freela", [], factory);
	else if(typeof exports === 'object')
		exports["Freela"] = factory();
	else
		root["Freela"] = factory();
})(window, function() {
return /******/ (function(modules) { // webpackBootstrap
/******/ 	// The module cache
/******/ 	var installedModules = {};
/******/
/******/ 	// The require function
/******/ 	function __webpack_require__(moduleId) {
/******/
/******/ 		// Check if module is in cache
/******/ 		if(installedModules[moduleId]) {
/******/ 			return installedModules[moduleId].exports;
/******/ 		}
/******/ 		// Create a new module (and put it into the cache)
/******/ 		var module = installedModules[moduleId] = {
/******/ 			i: moduleId,
/******/ 			l: false,
/******/ 			exports: {}
/******/ 		};
/******/
/******/ 		// Execute the module function
/******/ 		modules[moduleId].call(module.exports, module, module.exports, __webpack_require__);
/******/
/******/ 		// Flag the module as loaded
/******/ 		module.l = true;
/******/
/******/ 		// Return the exports of the module
/******/ 		return module.exports;
/******/ 	}
/******/
/******/
/******/ 	// expose the modules object (__webpack_modules__)
/******/ 	__webpack_require__.m = modules;
/******/
/******/ 	// expose the module cache
/******/ 	__webpack_require__.c = installedModules;
/******/
/******/ 	// define getter function for harmony exports
/******/ 	__webpack_require__.d = function(exports, name, getter) {
/******/ 		if(!__webpack_require__.o(exports, name)) {
/******/ 			Object.defineProperty(exports, name, { enumerable: true, get: getter });
/******/ 		}
/******/ 	};
/******/
/******/ 	// define __esModule on exports
/******/ 	__webpack_require__.r = function(exports) {
/******/ 		if(typeof Symbol !== 'undefined' && Symbol.toStringTag) {
/******/ 			Object.defineProperty(exports, Symbol.toStringTag, { value: 'Module' });
/******/ 		}
/******/ 		Object.defineProperty(exports, '__esModule', { value: true });
/******/ 	};
/******/
/******/ 	// create a fake namespace object
/******/ 	// mode & 1: value is a module id, require it
/******/ 	// mode & 2: merge all properties of value into the ns
/******/ 	// mode & 4: return value when already ns object
/******/ 	// mode & 8|1: behave like require
/******/ 	__webpack_require__.t = function(value, mode) {
/******/ 		if(mode & 1) value = __webpack_require__(value);
/******/ 		if(mode & 8) return value;
/******/ 		if((mode & 4) && typeof value === 'object' && value && value.__esModule) return value;
/******/ 		var ns = Object.create(null);
/******/ 		__webpack_require__.r(ns);
/******/ 		Object.defineProperty(ns, 'default', { enumerable: true, value: value });
/******/ 		if(mode & 2 && typeof value != 'string') for(var key in value) __webpack_require__.d(ns, key, function(key) { return value[key]; }.bind(null, key));
/******/ 		return ns;
/******/ 	};
/******/
/******/ 	// getDefaultExport function for compatibility with non-harmony modules
/******/ 	__webpack_require__.n = function(module) {
/******/ 		var getter = module && module.__esModule ?
/******/ 			function getDefault() { return module['default']; } :
/******/ 			function getModuleExports() { return module; };
/******/ 		__webpack_require__.d(getter, 'a', getter);
/******/ 		return getter;
/******/ 	};
/******/
/******/ 	// Object.prototype.hasOwnProperty.call
/******/ 	__webpack_require__.o = function(object, property) { return Object.prototype.hasOwnProperty.call(object, property); };
/******/
/******/ 	// __webpack_public_path__
/******/ 	__webpack_require__.p = "";
/******/
/******/
/******/ 	// Load entry module and return exports
/******/ 	return __webpack_require__(__webpack_require__.s = "./index.js");
/******/ })
/************************************************************************/
/******/ ({

/***/ "./index.js":
/*!******************!*\
  !*** ./index.js ***!
  \******************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

"use strict";


Object.defineProperty(exports, "__esModule", {
  value: true
});
var invalidos = function invalidos() {
  var items = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
  return items.every(function (item) {
    return item === 0 || isNaN(item) || typeof item !== 'number' && typeof item !== 'string';
  });
};

var Freela = {

  /*
   * @salarioMes ~> Quanto você quer ganhar por mês?
   * @diasSemana ~> Quantos dias você quer trabalhar por semana?
   * @horasDia ~> Quantas horas você quer trabalhar por dia?
   */
  hora: function hora() {
    var salarioMes = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;
    var diasSemana = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
    var horasDia = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;

    if (invalidos([salarioMes, diasSemana, horasDia])) return 0;
    return (parseFloat(salarioMes) / parseFloat(diasSemana) / parseFloat(horasDia)).toFixed(2);
  },

  /*
   * @valorHora ~> Quanto você ganha por hora?
   * @horasDia ~> Quantas horas irá trabalhar no projeto por dia?
   * @duracaoProjeto ~> Quantos dias irá durar o projeto?
   */
  valorProjeto: function valorProjeto() {
    var valorHora = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;
    var horasDia = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
    var duracaoProjeto = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;

    if (invalidos([valorHora, horasDia, duracaoProjeto])) return 0;
    return (parseFloat(valorHora) * parseFloat(duracaoProjeto) * parseFloat(horasDia)).toFixed(2);
  },

  /*
   * @valor ~> valor a ser formatado para real
   */
  formatarValor: function formatarValor(valor) {
    if (invalidos([valor])) valor = 0;
    if (typeof valor === 'string') valor = parseFloat(valor);
    var valorFormatado = valor.toFixed(2).split('.');
    valorFormatado[0] = 'R$ ' + valorFormatado[0].split(/(?=(?:...)*$)/).join('.');
    return valorFormatado.join(',');
  }
};

exports.default = Freela;
module.exports = exports.default;

/***/ })

/******/ });
});
//# sourceMappingURL=freela.js.map